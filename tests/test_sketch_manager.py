...
class TestSketchManager:
    ...
    def test_creates_new_sketch(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                sketch_path.name == "my_first_sketch.py"
            ), "Main file should be named my_first_sketch.py"
            assert (
                sketch_path.parent.name == "my_first_sketch"
            ), "Should be in sketch folder"
            assert (
                sketch_path.parent.parent == sketches_dir
            ), "Sketch folder should be in sketches directory"

            # Check folder structure
            sketch_folder = sketches_dir / "my_first_sketch"
            assert (
                sketch_folder.exists() and sketch_folder.is_dir()
            ), "Should create sketch folder"
            assert (
                sketch_folder / "my_first_sketch.py"
            ).exists(), "Should create my_first_sketch.py file"
...
"""
            ...
            assert sketch_path.exists(), "Sketch file should be created"
            assert (
                sketch_path.name == "red_rectangle.py"
            ), "Main file should be red_rectangle.py"
            assert (
                sketch_path.parent.name == "red_rectangle"
            ), "Should be in named folder"
...
"""
    ...
    def test_lists_sketches_by_category(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert any(
                "bouncing_ball.py" in str(sketch) for sketch in animation_sketches
            )
            assert any(
                "rotating_square.py" in str(sketch) for sketch in animation_sketches
            )
...
'''
    ...
    def test_creates_sketch_with_output_folder(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                output_folder / "render_001.png"
            ).exists(), "Output files should exist"
