...
class TestSecurityMiddleware:
    ...
    def test_multiple_client_rate_limiting(self):
        ...
        for client_ip in clients:
            for i in range(3):
                ...
                assert (
                    result.allowed
                ), f"Client {client_ip} request {i+1} should be allowed"
    ...
    def test_content_type_validation(self):
        ...
        valid_types = ["text/plain", "application/json", "text/html"]
    ...
    def test_security_middleware_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            app = FastAPI()
            config = SecurityConfig(
                allowed_sketch_directories=[project_dir], max_requests_per_minute=10
            )
            middleware = SecurityMiddleware(config)
