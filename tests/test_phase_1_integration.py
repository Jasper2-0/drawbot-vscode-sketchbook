...
'''
            ...
            assert len(available_versions) <= 3, "Should respect max_versions_per_sketch"
            ...
            assert versions[0] not in available_versions, "Oldest versions should be cleaned"
...
''')
            ...
            assert final_stats['total_size_mb'] <= final_stats['max_size_mb'] * 1.1  # Allow small tolerance
...
''')
            ...
            assert all(result.success for result in results), "All concurrent executions should succeed"
...
''')
            ...
            temp_files = [f for f in new_files if cache_dir not in f.parents and f.suffix != '.py']
            
            assert len(cache_files) > 0, "Should have cache files"
            assert len(temp_files) <= 1, "Should not accumulate temp files (max 1 output file)"