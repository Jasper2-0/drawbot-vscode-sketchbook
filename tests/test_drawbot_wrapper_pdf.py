...
class TestDrawBotWrapperPDF:
    ...
    def test_pdf_data_format_validation(self):
        ...
        assert pdf_data.startswith(
            b"%PDF-1."
        ), "Should start with valid PDF version header"
    ...
    def test_operation_recording_with_pdf(self):
        ...
        assert isinstance(
            pdf_data, bytes
        ), "Should generate PDF from recorded operations"
    ...
    def test_pdf_generation_error_handling(self):
        ...
        assert pdf_data.startswith(
            b"%PDF-"
        ), "Should return valid PDF header even on error"
    ...
    def test_pdf_generation_performance(self):
        ...
        assert (
            generation_time < 0.1
        ), f"PDF generation should be fast, took {generation_time:.3f}s"
