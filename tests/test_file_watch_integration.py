    ...
    async def test_debounced_rapid_file_changes(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            integration = FileWatchIntegration(
                project_path, cache_dir, mock_manager, debounce_delay=0.1
            )
...
"""
            ...
            assert any(
                msg_type in ["execution_started", "preview_updated"]
...
"""
            ...
            preview_messages = [
                call[0][1] for call in calls if call[0][1]["type"] == "preview_updated"
            ]
