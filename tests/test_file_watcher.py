...
class TestFileWatcher:
    ...
    def test_detect_sketch_file_change(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            found_sketch = any(Path(call).resolve() == sketch_file.resolve() for call in callback_calls)
            assert found_sketch, f"Should callback with modified file. Got: {callback_calls}, Expected: {sketch_file.resolve()}"
    ...
    def test_ignore_non_sketch_changes(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            found_other = any(Path(call).resolve() == other_file.resolve() for call in callback_calls)
    ...
    def test_debounce_rapid_changes(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert len(callback_calls) <= 2, f"Should debounce rapid changes, got {len(callback_calls)} callbacks"
            assert len(callback_calls) > 0, "Should still detect changes after debouncing"
    ...
    def test_change_callback_execution(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            call_args = callback_mock.call_args[0][0] if callback_mock.call_args else None
            if call_args:
    ...
    def test_callback_error_handling(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert len(working_calls) > 0, "Working callback should execute despite failing callback"
    ...
    def test_multiple_callback_registration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert sketch_file in callback1_calls, "First callback should receive file path"
            assert sketch_file in callback2_calls, "Second callback should receive file path"
    ...
    def test_watcher_shutdown(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert len(callback_calls) == initial_calls, "Should not detect changes after shutdown"
    ...
    def test_unwatch_file(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert len(callback_calls) == initial_calls, "Should not detect changes after unwatching"
