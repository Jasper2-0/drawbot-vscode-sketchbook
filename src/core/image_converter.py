...
class ImageConverter:
    ...
    def __init__(self, max_width: int = 1200, max_height: int = 1200, quality: int = 90, retina_scale: float = 3.0):
        ...
        try:
            ...
            if not pdf_data:
                return ConversionResult(
                    success=False,
                    error="Empty PDF data provided"
                )
            
            if len(pdf_data) < 10:
                return ConversionResult(
                    success=False,
                    error="PDF data too small to be valid"
                )
    ...
    def _convert_with_pymupdf(self, pdf_data: bytes, output_dir: Path) -> ConversionResult:
        ...
        try:
            ...
            # Calculate scaling for retina display quality
            page_rect = page.rect
            ...
            return ConversionResult(
                success=False,
                error=f"PyMuPDF conversion failed: {str(e)}"
            )
    ...
    def _convert_with_pil(self, pdf_data: bytes, output_dir: Path) -> ConversionResult:
        ...
        try:
            ...
            return ConversionResult(
                success=False,
                error=f"PIL conversion failed: {str(e)}"
            )
    
    def _generate_unique_filename(self, output_dir: Path, base_name: str = "preview") -> Path:
