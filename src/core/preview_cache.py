...
class PreviewCache:
    ...
    def __init__(self, cache_dir: Path, max_versions_per_sketch: int = 5, 
                 max_total_size_mb: float = 100, max_age_hours: int = 24):
        ...
        with self.lock:
            try:
                ...
                if not sketch_name or not sketch_name.strip():
                    return CacheResult(
                        success=False,
                        error="Invalid sketch name provided"
                    )
                ...
                return CacheResult(
                    success=False,
                    error=f"Failed to store preview: {str(e)}"
                )
    ...
    def get_preview_version(self, sketch_name: str, version: int) -> Optional[CacheEntry]:
    ...
    def _aggressive_cleanup(self):
        ...
        for created_at, sketch_name, entry in all_entries:
            ...
            try:
                ...
                if sketch_name in self.entries:
                    self.entries[sketch_name] = [
                        e for e in self.entries[sketch_name] 
