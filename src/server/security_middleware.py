...
class SecurityMiddleware:
            ...
            try:
                if "/" in ip_spec:
                    self.allowed_networks.append(
                        ipaddress.ip_network(ip_spec, strict=False)
                    )
                else:
                    self.allowed_networks.append(
                        ipaddress.ip_network(f"{ip_spec}/32", strict=False)
                    )
            except ValueError:
        ...
        try:
            ...
            if self._is_path_traversal(sanitized_name):
                return ValidationResult(
                    valid=False, error="Path traversal attempt detected"
                )
            ...
            project_path = self.config.allowed_sketch_directories[
                0
            ].parent  # Get project root
                ...
                for allowed_dir in self.config.allowed_sketch_directories:
                    if self._is_path_within_directory(sketch_path, allowed_dir):
                        ...
                        if self._matches_allowed_patterns(sanitized_name):
                            return ValidationResult(
                                valid=True, resolved_path=sketch_path
                            )
                        else:
                            return ValidationResult(
                                valid=False,
            ...
            return ValidationResult(
                valid=False, error=f"Path validation error: {str(e)}"
            )
    ...
    def validate_client_ip(self, client_ip: str) -> IPValidationResult:
        ...
        try:
            ...
            return IPValidationResult(
                allowed=False, error=f"IP address {client_ip} not in allowlist"
            )
            ...
            return IPValidationResult(
                allowed=False, error=f"Invalid IP address: {client_ip}"
            )
    ...
    def validate_content_type(self, content_type: str) -> ValidationResult:
        ...
        return ValidationResult(
            valid=False, error=f"Content type '{content_type}' not allowed"
        )
        ...
        class SecurityHTTPMiddleware(BaseHTTPMiddleware):
            async def dispatch(self, request: Request, call_next):
                ...
                if not ip_validation.allowed:
                    return Response(
                        content=f"Access denied: {ip_validation.error}", status_code=403
                    )
