    ...
    async def _handle_file_change(self, sketch_name: str, file_path: Path):
        ...
        async with self.execution_locks[sketch_name]:
            try:
                ...
                await self.preview_manager.broadcast_to_sketch(
                    sketch_name, {"type": "execution_started", "sketch": sketch_name}
                )
                ...
                if result.success:
                    ...
                    self.logger.info(
                        f"Sketch executed successfully: {sketch_name} "
                    ...
                    await self.preview_manager.broadcast_to_sketch(
                        sketch_name,
                        {
                            ...
                            "error": result.error,
                            "execution_time": (
                                result.execution_time if result.execution_time else 0
                            ),
                        },
                    )

                    self.logger.warning(
                        f"Sketch execution failed: {sketch_name} - {result.error}"
                    )
                ...
                self.logger.error(
                    f"Unexpected error handling file change for {sketch_name}: {e}"
                )
