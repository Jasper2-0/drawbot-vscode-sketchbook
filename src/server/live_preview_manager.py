...
class LivePreviewManager:
    ...
    async def connect_client(self, sketch_name: str, websocket: WebSocket):
        ...
        try:
            ...
            self.logger.info(f"Client connected to sketch '{sketch_name}'. "
            ...
            self.logger.debug(f"Current preview: {current_preview}, type: {type(current_preview)}")
            if current_preview:
                self.logger.debug(f"Sending preview with file_path: {current_preview.file_path}")
                await self._send_to_client(websocket, {
    ...
    async def disconnect_client(self, sketch_name: str, websocket: WebSocket):
        ...
        self.logger.info(f"Client disconnected from sketch '{sketch_name}'. "
    ...
    async def _cleanup_connection(self, websocket: WebSocket):
        ...
        if websocket in self.connection_info:
            ...
            if hasattr(connection_info, 'sketch_name'):
                ...
                self.logger.error(f"connection_info has no sketch_name attribute! Type: {type(connection_info)}")
                sketch_name = None
        ...
        try:
            if hasattr(websocket, 'client_state') and websocket.client_state.name == "OPEN":
    ...
    def get_connection_stats(self) -> Dict[str, Any]:
        ...
        """
        active_connections = sum(len(clients) for clients in self.active_connections.values())
    ...
    def is_watching_sketch(self, sketch_name: str) -> bool:
        ...
        """
        return sketch_name in self.active_connections and len(self.active_connections[sketch_name]) > 0
